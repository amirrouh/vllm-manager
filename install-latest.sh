...
# VLLM Manager Installer with Cache Busting
set -e
...
# Function to download with cache busting
download_with_cache_bust() {
    local url="$1"
    local output="$2"
    local cache_buster="?t=$TIMESTAMP"
    curl -fsSL "${url}${cache_buster}" -o "$output"
}
...
# Create pyproject.toml directly
...
# Use a simple echo instead of printf to avoid any parsing issues
echo ""
